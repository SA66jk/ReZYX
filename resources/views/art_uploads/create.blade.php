...
<body>
    <div class="upload-container">
        <form method="POST" action='{{ url("/museum") }}' enctype="multipart/form-data">
            @csrf
        ...
        @if ( ! empty($id) )
            <div class="uploaded-details">
                <p>Uploaded File:</p>
                <a href="{{ url('/museum', [$id, $originalName]) }}">{{ $id }} {{ $originalName }}</a>

                @if(substr($mimeType, 0, 5) == 'image')
                    <br>
                    <img height="100" width="100" src='{{ url('/museum', [$id, $originalName]) }}'>
                @endif
            </div>
        @endif

        <div class="uploaded-details">
            <a href="{{ url('/museum') }}">View Paint Art</a>
        </div>
